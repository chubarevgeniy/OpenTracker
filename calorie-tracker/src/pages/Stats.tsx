...
export default function Stats() {
 ...
 // ⚡ Bolt: Replace O(N log N) Object.keys().sort() with O(N) scan to find earliest date
 let earliestDate = "";
 ...
 if (!earliestDate || date < earliestDate) earliestDate = date;
      ...
      if (earliestDate) {
        daysToCalculate = differenceInDays(today, new Date(earliestDate)) + 1
 }
 ...
 if (data.length > 0) {
 // ⚡ Bolt: Replace O(N log N) sorted iteration with O(N) scan to find closest weight before range
 let closestDateBeforeRange = "";
 for (const date in dailyLogs) {
 if (date < data[0].fullDate && dailyLogs[date].weight) {
 if (!closestDateBeforeRange || date > closestDateBeforeRange) {
 closestDateBeforeRange = date;
 initialWeight = dailyLogs[date].weight;
 }
 }
 }
 }
 ...
 // ⚡ Bolt: Replace O(N log N) Object.keys().sort() with O(N) linear scan
      let closestDateBeforeToday = "";
      for (const date in dailyLogs) {
        if (date < todayStr && dailyLogs[date].weight) {
          if (!closestDateBeforeToday || date > closestDateBeforeToday) {
            closestDateBeforeToday = date;
